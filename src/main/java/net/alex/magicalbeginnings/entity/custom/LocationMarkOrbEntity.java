...
public class LocationMarkOrbEntity extends PersistentProjectileEntity {
    ...
    public LocationMarkOrbEntity(EntityType<? extends PersistentProjectileEntity> entityType, World world) {
    ...
    /**
     * Called when the projectile hits a block. Saves the coordinates of the block
     *
    ...
    protected void onBlockHit(BlockHitResult blockHitResult) {
        ...
        Vec3d coordinates = blockHitResult.getPos();

        this.discard();

        ClientPlayNetworking.send(ModMessages.TELEPORTATION_COORDINATE_X_UPDATE, PacketByteBufs.create().writeDouble(coordinates.getX()));
        ClientPlayNetworking.send(ModMessages.TELEPORTATION_COORDINATE_Y_UPDATE, PacketByteBufs.create().writeDouble(coordinates.getY()));
        ClientPlayNetworking.send(ModMessages.TELEPORTATION_COORDINATE_Z_UPDATE, PacketByteBufs.create().writeDouble(coordinates.getZ()));
    ...
    /**
     * Behaviour when the projectile hits an entity. Saves the coordinates of the entity
     *
    ...
    protected void onHit(LivingEntity target) {
        super.onHit(target);
        Vec3d coordinates = target.getPos();
